...
func createTask(wg *sync.WaitGroup, channel *amqp.Channel, qName string, parent int, delay time.Duration, amount int) {
	defer wg.Done()
	for i := 1; i <= amount; i++ {
		go func() {
			sgt := ""
			...
			msg := fmt.Sprintf("%vnew task for %v: this is message from parent: %v, child: %v.", sgt, qName, parent, i)
...
func run(channel *amqp.Channel) {
	...
	for i := min; i <= max; i++ {
		wg.Add(1)
		go createTask(&wg, channel, q.Name, i, ((time.Second / 4) * time.Duration(i)), ((max+1)*50)-(15*i))
	}
